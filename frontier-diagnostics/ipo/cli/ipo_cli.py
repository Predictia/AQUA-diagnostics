...
def parse_args(args):
    parser = argparse.ArgumentParser(description="IPO Tripole Index diagnostic")
...
def main(raw_args):
    ...
    try:
        ...
        print(f"Failed to import aqua or IPO diagnostic: {e}")
        print("Ensure the AQUA package and the IPO diagnostic are correctly installed/discoverable.")
        sys.exit(1)
    ...
    logger = log_configure(log_name="IPO CLI", log_level=loglevel)
    ...
    # Run the IPO diagnostic
    try:
        logger.info("Initializing IPO diagnostic...")
        diagnostic = IPO(config=config, loglevel=loglevel)

        logger.info("Retrieving data...")
        diagnostic.retrieve()

        logger.info("Computing IPO index...")
        ...
        logger.error("An error occurred during IPO diagnostic execution: %s", e, exc_info=True)
        logger.critical("IPO diagnostic failed.")
        sys.exit(1)


if __name__ == "__main__":
    print("Running IPO diagnostic CLI")
    main(sys.argv[1:])